...
export function ClientList({
  ...
  if (clients.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-black/10 bg-white/60 p-12 text-center">
  ...
  return (
    <>
      <div className="hidden overflow-hidden rounded-lg border border-black/5 bg-white md:block">
        <table className="w-full text-left text-sm">
          <thead className="bg-black/[.02] text-xs uppercase text-muted-foreground">
            <tr>
      ...
      <ul className="flex flex-col gap-3 md:hidden">
        {clients.map((c) => (
          <li key={c.id}>
            <Link
              href={`/clientes/${c.id}`}
              className="block rounded-lg border border-black/5 bg-white p-4 shadow-sm"
            >
