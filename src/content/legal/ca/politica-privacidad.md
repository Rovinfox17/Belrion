# Política de Privacitat

## 1. Responsable del tractament

- **Responsable:** Xavier Rosselló Díaz
- **NIF:** 79282716Z
- **Domicili:** Masia Can Rosselló-Can Ballesta, s/n, 08186 Lliçà d'Amunt, Barcelona
- **Correu electrònic de contacte:** soportebelrion@gmail.com

## 2. A quines dades s'aplica aquesta política

Belrion tracta dos tipus de dades personals diferents, amb rols diferents segons el RGPD:

### 2.1 Dades del compte de l'usuari de Belrion (tu)
Quan et registres i uses Belrion, es tracta el teu email i contrasenya (aquesta darrera xifrada, gestionada per Supabase Auth) per permetre't iniciar sessió de forma segura. Aquí, **Xavier Rosselló Díaz actua com a responsable del tractament**.

### 2.2 Dades dels clients que introdueixes a l'aplicació
Belrion et permet desar dades dels teus propis clients (nom d'empresa, persones de contacte, telèfon, email, adreça, productes contractats, notes de seguiment). **Sobre aquestes dades, tu ets el responsable del tractament** (ets qui decideix quines dades dels teus clients introdueixes i amb quina finalitat), i **Xavier Rosselló Díaz actua únicament com a encarregat del tractament**, és a dir, com el proveïdor tècnic que allotja i processa aquestes dades pel teu compte, sense decidir sobre la seva finalitat ni utilitzar-les per a finalitats pròpies.

> A la pràctica: si publiques o comparteixes Belrion amb altres persones, cadascuna serà responsable de les dades dels seus propis clients que introdueixi a l'aplicació, i haurà de comptar amb base legítima per tractar-les (per exemple, la relació comercial existent amb aquests clients).

## 3. Finalitat del tractament

- Gestionar el teu accés i autenticació a l'aplicació.
- Permetre't organitzar la teva cartera de clients, visites comercials i seguiment associat.
- Enviar-te notificacions/recordatoris de visites que tu mateix programes.
- Mantenir la seguretat i el correcte funcionament del servei.

No s'utilitzen les dades introduïdes per a finalitats publicitàries pròpies, ni se cedeixen a tercers diferents dels proveïdors tecnològics necessaris per prestar el servei (vegeu el punt 5).

## 4. Base legal

- **Dades del teu compte:** execució del contracte/condicions d'ús que acceptes en registrar-te.
- **Dades dels teus clients:** la base legal correspon a tu com a responsable d'aquestes dades (normalment, la relació contractual o precontractual amb els teus propis clients, o el seu consentiment, segons el cas).

## 5. Encarregats del tractament i proveïdors tecnològics

Per prestar el servei, Belrion es recolza en els següents proveïdors, que actuen com a encarregats del tractament sota els seus propis acords de protecció de dades (Data Processing Agreements):

| Proveïdor | Funció | Més informació |
|---|---|---|
| Supabase Inc. | Base de dades, autenticació i emmagatzematge | [supabase.com/privacy](https://supabase.com/privacy) |
| Vercel Inc. | Hosting i desplegament de l'aplicació | [vercel.com/legal/privacy-policy](https://vercel.com/legal/privacy-policy) |
| Resend, Inc. | Enviament d'emails transaccionals (sol·licituds d'accés, invitacions, recuperació de contrasenya) | [resend.com/legal/privacy-policy](https://resend.com/legal/privacy-policy) |
| Google LLC (Google Analytics) | Analítica d'ús de l'aplicació, només si acceptes les cookies d'analítica | [policies.google.com/privacy](https://policies.google.com/privacy) |

Aquests proveïdors poden emmagatzemar dades en servidors fora de l'Espai Econòmic Europeu, segons la regió que configuris. En el cas de Belrion, la base de dades de Supabase està allotjada a **eu-west-1 (Irlanda)**, és a dir, dins de la Unió Europea, per la qual cosa no hi ha transferència internacional de dades fora de l'EEE per a la informació emmagatzemada a Supabase. Si en el futur canvies de regió o de proveïdor, revisa si cal actualitzar aquest apartat i, si escau, incorporar les garanties adequades conforme al RGPD (com les Clàusules Contractuals Tipus de la Comissió Europea).

## 6. Termini de conservació

Les dades es conservaran mentre mantinguis el teu compte actiu a Belrion. Si elimines un client, contacte o visita des de la mateixa aplicació, s'elimina de la base de dades de forma permanent. Si decideixes tancar el teu compte, pots sol·licitar l'eliminació completa de les teves dades escrivint a soportebelrion@gmail.com.

## 7. Els teus drets

Com a titular de les teves pròpies dades (el teu compte), tens dret a accedir, rectificar, suprimir, limitar el tractament, oposar-te i sol·licitar la portabilitat de les teves dades, escrivint a **soportebelrion@gmail.com**. També tens dret a presentar una reclamació davant l'Agència Espanyola de Protecció de Dades (AEPD) si consideres que els teus drets no han estat atesos correctament ([www.aepd.es](https://www.aepd.es)).

Si ets una persona les dades de la qual han estat introduïdes per un usuari de Belrion (per exemple, ets el contacte d'una empresa client registrada a l'aplicació d'un comercial), has de dirigir la teva sol·licitud directament a aquesta persona o empresa, ja que és qui actua com a responsable de les teves dades.

## 8. Mesures de seguretat

Belrion aplica mesures tècniques i organitzatives raonables per protegir les dades, incloent-hi:

- Xifratge de contrasenyes i comunicacions (HTTPS/TLS).
- Row Level Security (RLS) a la base de dades, que impedeix que un usuari accedeixi a les dades d'un altre.
- Accés restringit a les claus i secrets de configuració del servei.

## 9. Canvis en aquesta política

Aquesta política es podrà actualitzar per adaptar-se a canvis legislatius o del mateix servei. Es recomana revisar aquesta pàgina periòdicament.

---
*Darrera actualització: 31 d'agost de 2026*
